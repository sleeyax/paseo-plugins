...
The plugin is nothing but data: four `addTheme` calls, no surface and no `server.handle`, so there is no server entry, and `pnpm test` only checks that `contribute` registers every flavour.
